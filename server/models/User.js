...
userSchema.pre('save',function(next){ //save함수가 실행되기전에무엇을한다는의미
    var user = this;
    ...
    } else { //비밀번호를변경하지않을땐 그냥바로다음실행
        next(); 
    }
})
userSchema.methods.comparePassword = function(planePassword,cb){
    bcrypt.compare(planePassword,this.password,function(err,isMatch){//isMatch는 맞으면true를반환
...
userSchema.statics.findByToken = function(token,cb){
    ...
    //토큰을 decode한다.
    jwt.verify(token,'secretToken',function(err,decoded){//decoded:복호화된_id값
